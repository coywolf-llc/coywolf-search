...
final class Coywolf_Search_GitHub_Updater {

	const REPO          = 'coywolf-llc/coywolf-search';
	const TRANSIENT_KEY = 'coywolf_search_gh_release';
	const CACHE_HOURS   = 6;
	 ...
	 */
	const CRON_HOOK = 'coywolf_search_refresh_release';
	...
	public function maybe_show_update_error() {
		...
		printf(
			...
			esc_html__( 'Coywolf Search:', 'coywolf-search' ),
			esc_html(
				sprintf(
					/* translators: %s: failure reason */
					__( 'could not check GitHub for updates (%s). This is usually temporary GitHub API rate-limiting on your host; the check retries automatically. If it persists, install the latest release manually from the plugin\'s GitHub Releases page.', 'coywolf-search' ),
					$err
				)
			)
		);
	...
	public function guard_pre_download( $reply, $package, $upgrader, $hook_extra = array() ) {
		...
		if ( ! is_string( $package ) || '' === $this->validate_package_url( $package ) ) {
			return new WP_Error(
				'coywolf_search_untrusted_package',
				__( 'Refusing to download a plugin update from an untrusted host.', 'coywolf-search' )
			);
		}
		return $reply;
	}
		...
		foreach ( $plugin_meta as $i => $item ) {
			...
			if ( false !== strpos( $item, 'plugin-install.php?tab=plugin-information' )
				...
				$plugin_meta[ $i ] = sprintf(
					...
					esc_url( $repo_url ),
					esc_html__( 'View details', 'coywolf-search' )
				);
			}
		}
		return $plugin_meta;
	...
	public function plugin_info( $result, $action, $args ) {
		...
		$info->sections      = array(
			'description' => 'Replaces WordPress search with a custom full-text index: BM25 ranking, fuzzy and prefix matching, and instant typeahead.',
	...
	public function fix_source_dirname( $source, $remote_source, $upgrader, $hook_extra = array() ) {
		...
		return new WP_Error(
			'coywolf_search_rename_failed',
			__( 'Could not rename the downloaded update folder to match the plugin slug.', 'coywolf-search' )
		);
	}
